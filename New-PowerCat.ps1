...
[CmdletBinding()]
    ...
    DynamicParam {
        ...
        if (($Mode -eq 'Tcp') -or ($Mode -eq 'Udp')) {
            $PortParam = New-RuntimeParameter -Name Port -Type Int -Mandatory
    ...
    if ($PSCmdlet.ParameterSetName -eq 'Client') {
        ...
        if ($Client -notmatch $IPv4) { # Try to resolve IP address from hostname

            try { $AddressList = [Net.Dns]::GetHostByName($Client).AddressList }
            catch { 
                Write-Warning "Unable to resolve IP address for $Client."
                break
            }
        ...
        switch ($Mode) {
            'Tcp' { $TcpClient = New-TcpClient -ServerIp $ServerIPAddress -Port $Port }

            'Udp' { $UdpClient = New-UdpClient -ServerIp $ServerIPAddress -Port $Port }

            'Smb' { $SmbClient = New-SmbClient -Server $Client -PipeName $PipeName }

            'Icmp' { $IcmpClient = New-IcmpClient -Server $Client }
        }
    }

    else { # We're setting up a listener/server
        
        switch ($Mode) {
            'Tcp' {
                if (!(Test-Port $Port)) { break }
                $TcpClient = New-TcpClient -Listener -Port $Port
            }

            'Udp' {
                if (!(Test-Port $Port)) { break }
                $UdpClient = New-UdpClient -Listener -Port $Port
            }

            'Smb' { $SmbClient = New-SmbClient -Listener -PipeName $PipeName }

            'Icmp' { $IcmpClient = New-IcmpClient -Listener -ListenOn $IcmpServer }
        }  
    }

    if ($PSBoundParameters.Relay) {
        
        if ($Relay.Split(":").Count -eq 2) {

            $RelayPort = [Int]$Relay.Split(":")[1]
            if (!(Test-Port $RelayPort)) { break }
        }        
        elseif ($Relay.Split(":").Count -eq 3) {

            $RelayPort = [Int]$Relay.Split(":")[2]
            if (!(Test-Port $RelayPort)) { break }
        }
    }
